...
export const adminDashboardRouter = createTRPCRouter({
  stats: adminProcedure.query(async ({ ctx }) => {
    ...
    const [
      ...
      ctx.db.bookingItem.count({ where: { status: "CHECKED_IN" } }),
    ]);

    const revenueMonthValue = Number(revenueMonth._sum.totalAmount ?? 0);
    const revenuePrevMonthValue = Number(
      revenuePrevMonth._sum.totalAmount ?? 0,
    );
    const revenueGrowth =
      revenuePrevMonthValue > 0
        ? ((revenueMonthValue - revenuePrevMonthValue) /
            revenuePrevMonthValue) *
          100
        : null;
    const bookingGrowth =
      totalBookingsPrevMonth > 0
        ? ((totalBookingsMonth - totalBookingsPrevMonth) /
            totalBookingsPrevMonth) *
          100
        : null;
  ...
  recentBookings: adminProcedure.query(({ ctx }) =>
  ...
  analyticsReport: adminProcedure.query(async ({ ctx }) => {
    const now = new Date();
    const monthStart = startOfMonth(now);
    const monthEnd = endOfMonth(now);
    const prevMonthStart = startOfMonth(subDays(monthStart, 1));
    ...
    const hotelIds = topHotels.map((r) => r.hotelId);
    const hotels = await ctx.db.hotel.findMany({
      where: { id: { in: hotelIds } },
    ...
    for (const b of revenueByDay) {
      ...
      if (key in revenueMap) revenueMap[key] += Number(b.totalAmount);
    }

    const avgValue = Number(avgBookingValue._avg.totalAmount ?? 0);
    const avgValuePrev = Number(avgBookingValuePrev._avg.totalAmount ?? 0);
    const avgValueGrowth =
      avgValuePrev > 0
        ? ((avgValue - avgValuePrev) / avgValuePrev) * 100
        : null;
    ...
    return {
      ...
      revenueByDay: days.map((date) => ({ date, revenue: revenueMap[date]! })),
      topHotels: topHotels.map((r) => ({
        name: hotelMap[r.hotelId]?.name ?? "—",
        revenue: Number(r._sum.totalAmount ?? 0),
        bookings: r._count.id,
      })),
