...
export function WriteReviewClient({ bookingRef }: WriteReviewClientProps) {
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="rounded-2xl border bg-card p-8 flex flex-col items-center gap-3 text-center">
          ...
          <h2 className="font-semibold">Bạn đã gửi đánh giá này rồi</h2>
  ...
  if (data.status !== "CHECKED_OUT") {
    return (
      <div className="space-y-5">
        <BackLink bookingRef={bookingRef} />
        <div className="rounded-2xl border bg-muted/40 p-8 flex flex-col items-center gap-3 text-center">
      ...
      <div className="rounded-2xl border bg-card p-4 space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm">{data.hotel.name}</span>
          </div>
          {item && (
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>
              {format(new Date(data.checkIn), "dd/MM/yyyy", { locale: vi })}
              {" → "}
              {format(new Date(data.checkOut), "dd/MM/yyyy", { locale: vi })}
            </span>
          </div>
        </div>
      </div>
